...
def load(): 
    class Calendar:
        ...
        def selection(self, day, name):
            ...
            global SelectedYear
            SelectedDay = day
            SelectedMonth = calendar.month_name[self.month_selected]
            SelectedYear = (self.year)
            self.clear()
            self.setup(self.year, self.month)
        ...
        def setup(self, y, m):
            ...
            ok = tk.Button(self.parent, width=5, text='OK', command=self.ReturnDate)
            self.wid.append(ok)
            ok.grid(row=9, column=2, columnspan=3, pady=10)
 
            
        def ReturnDate(self):
            from NewFlight import EnterDate
            EnterDate(root, SelectedDay, SelectedMonth, SelectedYear)
     
        
    class Control:
        def __init__(self, parent):
            self.data = {}
            cal = Calendar(root, self.data)
